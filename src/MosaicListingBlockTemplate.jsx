...
const MosaicListingBlockTemplate = ({
    ...
    const bentoGroups = useMemo(() => groupIntoBento(processedItems), [processedItems]);
    ...
    const getImageSrc = (item, size) =>
        item.image_field ? `${item.url}/@@images/${item.image_field}/${size}` : null;
                    ...
                    {imageSrc ? (
                        <img
                            ...
                            srcSet={[
                                ...
                                `${item.url}/@@images/${item.image_field}/large 900w`,
                                `${item.url}/@@images/${item.image_field}/larger 1200w`,
                            ].join(',')}
                            sizes="(max-width: 767px) 100vw, 50vw"
                            alt={item.title || ''}
                            className="mosaic-card__image"
                            loading={fetchPriority === 'high' && globalItemIdx === 0 ? 'eager' : 'lazy'}
                            fetchpriority={globalItemIdx === 0 ? fetchPriority || 'auto' : 'auto'}
                        />
                    ) : (
                        <div className="mosaic-card__image--placeholder" aria-hidden="true" />
                    )}
                </div>

                {/* Panel */}
                <div className="mosaic-card__panel" style={{ backgroundColor: combo.bg }}>

                    <TitleTag id={ariaId} className="mosaic-card__title" style={{ color: combo.text }}>
                        {isEditMode ? (item.title || item.id) : (
                            <Link
                                to={item.url}
                    ...
                    {showDescription && item.description && (
                        <p className="mosaic-card__description">
                            {item.description}
                        </p>
                    )}
    ...
    const renderSmallCard = (item, combo, globalItemIdx) => {
        ...
        const card = (
            ...
            >
                {/* Image — left side */}
                {imageSrc ? (
                    <div className="mosaic-card__image-wrap mosaic-card__image-wrap--side" aria-hidden="true">
                        <img
                            src={imageSrc}
                            srcSet={[
                    ...
                    <div className="mosaic-card__image-wrap mosaic-card__image-wrap--side mosaic-card__image--placeholder" aria-hidden="true" />
                )}

                {/* Panel — right side */}
                <div className="mosaic-card__panel mosaic-card__panel--side" style={{ backgroundColor: combo.bg }}>

                    <TitleTag id={ariaId} className="mosaic-card__title mosaic-card__title--small" style={{ color: combo.text }}>
                        {isEditMode ? (item.title || item.id) : (
                            <Link
                                to={item.url}
                                aria-label={item.title || item.id}
    ...
    return (
        <section className="mosaic-listing" aria-label={header || 'Featured articles'}>
            {headerLink && (
            ...
            <ul className="mosaic-bento" role="list">
                {bentoGroups.map((group, groupIdx) => {
                    ...
                    const smallStack = group.smalls.length > 0 ? (
                        <li className="mosaic-bento__small-stack" key={`stack-${groupIdx}`} aria-label="Related articles">
                            <ul className="mosaic-bento__smalls" role="list">
                                {group.smalls.map((item, si) => (
                                    <li key={item['@id']} className="mosaic-bento__small-item">
                                        {renderSmallCard(item, smallCombos[si], largeIdx + si + 1)}
                                    </li>
                                ))}
                            </ul>
                        </li>
                    ...
                    return (
                        <li
                            key={`group-${groupIdx}`}
                            className={`mosaic-bento__group ${largeOnRight ? 'mosaic-bento__group--reversed' : ''}`}
                        >
                            <ul className="mosaic-bento__group-inner" role="list">
                                {largeOnRight ? [smallStack, largeCard] : [largeCard, smallStack]}
                            </ul>
                        </li>
                    );
                })}
            ...
            {moreLink && (
                <div className="mosaic-listing__more-link">{moreLink}</div>
            )}
        </section>
    );
};
